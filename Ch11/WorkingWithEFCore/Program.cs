...
namespace WorkingWithEFCore
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //     WriteLine("Update product price sucessful.");
            // }
        ...
        static void QueryingProducts(){
            using(var db=new Northwind()){
                ...
                decimal price;
                do{
                    Write("Enter a priduct price: ");
                    input = ReadLine();

                }while(!decimal.TryParse(input,out price));

                IOrderedEnumerable<Product> prods =db.Products.AsEnumerable()
                    .Where(prod =>prod.Cost > price)
                    .OrderByDescending(prod =>prod.Cost);
